...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      for j in range(num_test):
        dists[i, j] = torch.sum((x_train[i]-x_test[j])**2)**(1/2)
        # Or just use: dists[i, j] = torch.norm(x_train[i] - x_test[j])
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    _, closest_idx = torch.topk(dists, k=k, dim=0, largest=False)
    for i in range(num_test):
      _, idx = torch.max(y_train[closest_idx[:, i]].bincount(), dim = 0)
      y_pred[i] = idx
...
def knn_cross_validate(
    ...
    # Split the tensor into 5 chunks along the 0th dimension (rows)
    ...
    for k in k_choices:
      ...
      for num_fold in range(num_folds):
        x_train_folds_copy = [x for x in x_train_folds]
        y_train_folds_copy = [x for x in y_train_folds]
        x_test = x_train_folds_copy[num_fold]
        y_test = y_train_folds_copy[num_fold]
        del x_train_folds_copy[num_fold]
        del y_train_folds_copy[num_fold]
        x_train = torch.cat(x_train_folds_copy, dim = 0)
        y_train = torch.cat(y_train_folds_copy, dim = 0)
        model = KnnClassifier(x_train, y_train)
        list_of_acc.append(model.check_accuracy(x_test, y_test, k))
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k,accs in sorted(k_to_accuracies.items()):
      avg_acc_dict[k] = sum(accs)/len(accs)
    best_k = max(avg_acc_dict, key=avg_acc_dict.get)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
