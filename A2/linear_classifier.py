...
def svm_loss_naive(
    ...
    num_classes = W.shape[1] # in total C classes, I don't get why W is transposed originally
    num_train = X.shape[0] # N rows means N training samples
    loss = 0.0
    for i in range(num_train): # loop N images, each image we get loss and gradient
        scores = W.t().mv(X[i]) # (C, 1)
        # each row of W.t() is multiplied by X[i] & each row of X is an image
        correct_class_score = scores[y[i]]
        for j in range(num_classes): # loop C classes, for incorrect classes, we get loss and gradient
            # next line is the fomula of SVM loss, referring to DL-3 P44
            if j == y[i]:
                continue
            ...
            if margin > 0:
                ...
                # dL/dW = dL/dS * dS/dW = M/dS * dS/dW
                # M: margin/L: loss/S: score, first do partial derivative
                dL_dS = 1 # dL/dS, here S is wrong class score
                dL_dcS = -1 # dL/dS, here S is correct class score

                dS_dW = X[i] # dS/dW: X*W = S
                # Here dW is actually dL/dW(gradient)
                dW[:, j] += dL_dS * dS_dW # wrong label class gradient descent
                dW[:, y[i]] += dL_dcS * dS_dW # correct label class gradient descent
    ...
    loss /= num_train
    dW /= num_train
    # Add regularization to the loss. DL-3/P 64
...
def svm_loss_vectorized(
    ...
    scores = X @ W # (N, C) torch.Size([128, 10]), each row of scores is one score record for one image
    correct_class_score = scores[range(num_train),y] # (N, C) torch.Size([128]), for row[i], get the y[i]th element
    margin = scores -  correct_class_score.view(-1,1) + 1 # (N, C) each column of scores - correct_class_score + 1
    margin[range(num_train),y] = 0 # (N, C) after previous line, these cells are 1, should be set to 0
    margin = torch.clamp(margin, min=0) # (N, C) convert values that are less than 0 to 0
    loss = margin.sum() / num_train + reg * torch.sum(W * W) # (N, C)
    
    # Gradient descent: for the code above, we write back propagation we write code from bottom to up
    # dL/dW = dL/d_L * d_L/dM * (dM/dS1 + dM/dS2) * dS/dW
    dL_dS = torch.zeros_like(margin); # (N, C)
    dL_dS[margin > 0] = 1; # line 248
    dL_dS[range(num_train), y] = 0
    dL_dS[range(num_train), y] -= torch.sum(dL_dS, axis = 1)

    dW = X.T @ dL_dS / num_train # (D, C), X.T is dS/dW
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    _, y_pred = X.mm(W).max(dim=1) # element in y_pred will be index
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_pred
...
def softmax_loss_naive(
    ...
    C = W.shape[1]
    for i in range(N):
      score = X[i] @ W # (1, C), score for current image
      score -= torch.max(score) # stablization
      score = torch.exp(score)
      score_sum = torch.sum(score)
      score /= score_sum
      loss -= torch.log(score[y[i]])
      for j in range(C):
        if j == y[i]:
          dW[:,j] += X[i] * (score[j] - 1)
        else:
...
def softmax_loss_vectorized(
    ...
    N = X.shape[0]
    S = X @ W # (N, C), S: scores
    S_max,_ = torch.max(S, axis=1) # second dimension is index fo max value
    S -= S_max.view(-1,1)
    S = torch.exp(S)
    sum_S = torch.sum(S, axis=1)
    S /= sum_S.view(-1,1)

    loss = -1*torch.sum(torch.log(S[range(N), y]))

    dL_dS = S
    dL_dS[range(N), y] -= 1

    dW = X.T @ dL_dS # dL/dW = dL/dS * dS/dW
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = torch.linspace(1e-2, 2e-1, 10)
    regularization_strengths = torch.linspace(1e-4, 1e-3, 10)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
